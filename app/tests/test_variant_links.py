...
def test_avi_declares_its_link_and_the_display_reads_it():
    """The two halves have to agree: the parse produces `avi.link`, and the rows
    point at it. The spec's own consistency check covers the reverse — a row
    naming a field nothing produces fails to load."""
