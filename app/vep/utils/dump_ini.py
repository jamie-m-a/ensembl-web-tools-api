...
Nextflow/Seqera pipeline, so the "form -> ini" stage can be driven end to end
from the frontend without pipeline credentials or a real run.

Upstream deleted this in 68898ea ("Remove dev mode, cleanup"); it is kept on
this fork only. See the LOCAL DEV HARNESS block in ``core/config.py``.
"""
